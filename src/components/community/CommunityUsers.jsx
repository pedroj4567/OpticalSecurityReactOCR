...
const CommunityUsers = ({ fetchData, response, loading, error }) => {
  ...
  const updateMethod = async (id, body) => {
    try {
      ...
      const { data } = await axios.post(`${baseURL}/${id}`, body);
      console.log("hola", data);
                ...
                return (
                  <>
                    <tr class="bg-white border-b text-gray-700 dark:border-purple-600">
                      ...
                      <td class="px-6 py-4">{user.n_phone}</td>
                      <td class="px-6 py-4">{user.identification}</td>

                      <td class="px-6 py-4 flex items-center gap-3">
                        <button
                          onClick={() => openEditForm(user.uuid)}
                          className="font-medium bg-blue-600 p-1 rounded text-white hover:bg-blue-400 flex items-center"
                        >
                        ...
                        <button
                          ...
                          className="font-medium p-1 rounded bg-red-600 text-white  hover:bg-red-400 flex items-center"
                        >
